...
export interface IGetEmailVerificationTokenService extends IServiceHandlerAsync<IGetEmailVerificationTokenServiceParameters,UserSettingsEntity> {}
...
export class GetEmailVerificationTokenService implements IGetEmailVerificationTokenService {
  ...
  public async handleAsync(params: IGetEmailVerificationTokenServiceParameters): Promise<Result<UserSettingsEntity, ResultError>> {
    try
    {
      if(!params)
      ...
      if(!params.userSettingsEntity)
        return ResultExceptionFactory.error(StatusCodes.BAD_REQUEST,"Invalid user setting entity");
