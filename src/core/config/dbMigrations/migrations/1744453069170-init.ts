...
export class Init1744453069170 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "IDX_6c3a73bbc9d8a8082816adc870" ON "user"."users" ("clientId") `
		);
		await queryRunner.query(
			`CREATE TYPE "user"."usersCommunication_status_enum" AS ENUM('1', '0')`
		);
		await queryRunner.query(
			`CREATE TABLE "user"."usersCommunication" ("id" BIGSERIAL NOT NULL, "identifier" character varying(50) NOT NULL, "status" "user"."usersCommunication_status_enum" NOT NULL DEFAULT '0', "created_date" TIMESTAMP(6) NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "modified_date" TIMESTAMP(6) NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "version" integer NOT NULL, "email" character varying(100) NOT NULL, "mobileNo" character varying NOT NULL, "userId" character varying NOT NULL, CONSTRAINT "REL_7f2e9f01037ffe01c50a9ab95a" UNIQUE ("userId"), CONSTRAINT "PK_780582e4bb7f33758ee98f6feba" PRIMARY KEY ("id"))`
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "IDX_5991e19ab3c15fc655d82767ac" ON "user"."usersCommunication" ("identifier") `
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "IDX_b99ca9c360cc1c3c62143dae84" ON "user"."usersCommunication" ("email") `
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "IDX_d579c88842376df82ed766f831" ON "user"."usersCommunication" ("mobileNo") `
		);
		...
		await queryRunner.query(
			`ALTER TABLE "user"."usersKeys" ADD CONSTRAINT "FK_8dad5622e2872163359b06dcb32" FOREIGN KEY ("userId") REFERENCES "user"."users"("identifier") ON DELETE NO ACTION ON UPDATE NO ACTION`
		);
		await queryRunner.query(
			`ALTER TABLE "user"."usersCommunication" ADD CONSTRAINT "FK_7f2e9f01037ffe01c50a9ab95a7" FOREIGN KEY ("userId") REFERENCES "user"."users"("identifier") ON DELETE NO ACTION ON UPDATE NO ACTION`
		);
	}

	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`ALTER TABLE "user"."usersCommunication" DROP CONSTRAINT "FK_7f2e9f01037ffe01c50a9ab95a7"`
		);
		await queryRunner.query(
			`ALTER TABLE "user"."usersKeys" DROP CONSTRAINT "FK_8dad5622e2872163359b06dcb32"`
		);
		await queryRunner.query(
